...
class TestHypoEvolveEvaluator(unittest.TestCase):
    ...
    def test_build_evaluator_prompt_variables_include_expected_dataset_and_hypothesis_fields(self):
    ...
    def test_build_evaluator_runtime_wrapper_embeds_runtime_bootstrap_and_parameters(self):
    ...
    def test_build_steering_prompt_variables_include_recent_history_and_top_hypothesis_section(self):
        top_hypotheses = [
            ArchiveEntry(
    ...
    def test_evaluate_returns_normalized_metrics_on_success(self):
        llm = FakeLLMClient(
            outputs=[
    ...
    def test_evaluate_retries_after_runtime_keyerror_with_previous_code(self):
        llm = FakeLLMClient(
            outputs=[
                "def evaluate_hypothesis(accessor: DatasetAccessor, parameters: dict[str, object] | None = None) -> dict[str, object]:\n"
    ...
    def test_evaluate_returns_failure_metrics_after_exhausted_retries(self):
        ...
        self.assertEqual(evaluator.last_evaluation_artifacts["attempt"], 2)
        self.assertIn("def still_broken(", evaluator.last_evaluation_artifacts["candidate_code"])

    def test_evaluate_sanitizes_non_finite_metrics(self):
        llm = FakeLLMClient(
            outputs=[
                "def evaluate_hypothesis(accessor: DatasetAccessor, parameters: dict[str, object] | None = None) -> dict[str, object]:\n"
    ...
    def test_evaluate_fills_missing_required_fields(self):
        llm = FakeLLMClient(
            outputs=[
                "def evaluate_hypothesis(accessor: DatasetAccessor, parameters: dict[str, object] | None = None) -> dict[str, object]:\n"
    ...
    def test_evaluate_uses_rendered_prompt_variables_and_runtime_wrapper_to_execute_candidate(self):
        llm = FakeLLMClient(outputs=["def evaluate_hypothesis():\n    return {}"])
        evaluator = LLMEvaluator(
            llm,
            self.schema,
            "dataset.yaml",
        ...
        self.assertEqual(metrics["combined_score"], 0.1)
        load_prompt_mock.assert_called_once_with("evaluator", "system.md")
        render_prompt_mock.assert_called_once()
        self.assertEqual(render_prompt_mock.call_args.args, ("evaluator", "user.md"))
        self.assertIn("variables", render_prompt_mock.call_args.kwargs)
        wrapper_mock.assert_called_once_with("dataset.yaml", None)

    def test_evaluate_stores_candidate_artifacts_in_last_evaluation_artifacts_before_execution(self):
        llm = FakeLLMClient(
            outputs=[
                "def evaluate_hypothesis(accessor: DatasetAccessor, parameters: dict[str, object] | None = None) -> dict[str, object]:\n"
                "    return {'combined_score': 0.5, 'precision': 0.7, 'baseline': 0.2, 'coverage': 0.4, 'uplift': 0.5, 'support_count': 2, 'total_count': 5, 'rationale': 'ok', 'used_parameters': {}}\n"
            ]
        )
        executor = FakeExecutor(
            results=[
        ...
        evaluator.evaluate(self.hypothesis)

        self.assertEqual(evaluator.last_evaluation_artifacts["attempt"], 1)
        self.assertIn("candidate_code", evaluator.last_evaluation_artifacts)
        self.assertIn("wrapper_code", evaluator.last_evaluation_artifacts)

    def test_evaluate_normalizes_invalid_payload_types_to_defaults(self):
        llm = FakeLLMClient(outputs=["def evaluate_hypothesis():\n    return {}"])
        evaluator = LLMEvaluator(
            llm,
            self.schema,
        ...
        payload = evaluator.evaluate(self.hypothesis)

        self.assertEqual(payload["combined_score"], 0.0)
        self.assertEqual(payload["precision"], 0.0)
        self.assertEqual(payload["baseline"], 0.2)
        self.assertEqual(payload["coverage"], 0.0)
        self.assertEqual(payload["uplift"], 1.5)
        self.assertEqual(payload["support_count"], 0)
        self.assertEqual(payload["total_count"], 0)
        self.assertEqual(payload["rationale"], "")
        self.assertEqual(payload["used_parameters"], {})

    def test_evaluate_returns_failure_metrics_when_executor_stdout_is_not_json_object(self):
        llm = FakeLLMClient(outputs=["def evaluate_hypothesis():\n    return {}"], retries=0)
        executor = FakeExecutor(
            results=[
                ExecutionResult(
        ...
        self.assertEqual(payload["combined_score"], 0.0)
        self.assertIn("JSON object", payload["rationale"])

    def test_evaluate_records_work_dir_in_last_evaluation_artifacts_on_success(self):
        llm = FakeLLMClient(outputs=["def evaluate_hypothesis():\n    return {}"])
        executor = FakeExecutor(
            results=[
                ExecutionResult(
                    stdout='{"combined_score": 0.1}',
                    stderr='',
                    exit_code=0,
                    timed_out=False,
                    duration_sec=0.02,
        ...
        self.assertEqual(payload["combined_score"], 0.1)
        self.assertEqual(payload["used_parameters"], {})
        self.assertEqual(evaluator.last_evaluation_artifacts["work_dir"], "/tmp/workdir")


    def test_build_evaluator_prompt_variables_renders_missing_column_descriptions_cleanly(self):
        schema = DatasetSchema(
            files=[DataFile(entity="BTCUSDT", path="BTCUSDT.parquet")],
            index=IndexSpec(name="close_time", dtype="datetime64[us]"),
            columns=[ColumnSpec(name="CLOSE", description=""), ColumnSpec(name="VOLUME", description=None)],
            description="Test dataset",
        )
        accessor = DatasetAccessor(schema)
        variables = build_evaluator_prompt_variables(self.hypothesis, schema, accessor)
        self.assertIn('- CLOSE:', variables['COLUMN_SPECS'])
        self.assertIn('- VOLUME:', variables['COLUMN_SPECS'])
        self.assertNotIn('None', variables['COLUMN_SPECS'])

    def test_build_evaluator_runtime_wrapper_emits_none_literal_when_parameters_absent(self):
        wrapper = build_evaluator_runtime_wrapper('dataset.yaml', None)
        self.assertIn('parameters = None', wrapper)
        self.assertIn('load_dataset_schema', wrapper)

    def test_build_steering_prompt_variables_preserves_recent_history_json_and_top_hypothesis_metadata_section(self):
        top_hypotheses = [
            ArchiveEntry(
                hypothesis=Hypothesis(root=AtomicNode('BEST')),
                metrics={'combined_score': 0.9},
        ...
        variables = build_steering_prompt_variables(
            ...
            recent_history=[{'mutation_summary': 'replace', 'score_delta': 0.2}],
            top_hypotheses=top_hypotheses,
        )
